...
pnpm --filter @repo/api exec medusa user --email admin@example.com --password yourpassword
...
```bash
pnpm --filter @repo/api docker:down   # stop PostgreSQL + Redis
pnpm --filter @repo/api migrate       # run pending migrations
pnpm --filter @repo/api seed          # seed demo data (run once manually)
pnpm --filter @repo/api typecheck     # TypeScript check
pnpm --filter @repo/api format        # format with Prettier
```
